...
export default async function eatDefineProcedureReturn(astInfo) {
  const { ast, variables, procedures, stacktrace } = astInfo;
  if (ast.type !== "procedures_defreturn") {
    throw new Error(`Not procedures_defreturn Block (${ast.type})`);
  }

  const { NAME } = ast.fields;
  const { STACK, RETURN } = ast.inputs || {};
  ...
  procedures[NAME] = async (localParams) => {
    ...
    let value;
    const result = await eatToken(eatCommandTerm, STACK, astInfo, {
      stopReturn: false,
      value: undefined,
    });
    ...
    if (!result.stopReturn) {
      value = await eatToken(eatValueTerm, RETURN, astInfo);
    } else {
      value = result.value;
    }
