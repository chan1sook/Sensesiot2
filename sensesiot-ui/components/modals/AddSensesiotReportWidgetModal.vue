  ...
  >
    <b-row>
      <b-col class="d-flex flex-column" style="gap: 0.5em">
        <div
          v-for="widgetData of reportWidgetsDescription"
          :key="widgetData.name"
        >
          <b-button
            type="button"
...
<script>
import { reportWidgetsDescription } from '~/utils/report'
...
export default {
  ...
  computed: {
    reportWidgetsDescription() {
      return reportWidgetsDescription
    },
    selectedWidget() {
      if (this.selectedWidgetName) {
        const target = this.reportWidgetsDescription.find(
  ...
  methods: {
    creditCostOf(type) {
      return (
        this.creditCosts.reportWidget[type] ||
        this.creditCosts.reportWidget.others ||
        0
      )
    },
